...
This specification defines 14 agentic workflows across 4 agent personas, following the OODA (Observe-Orient-Decide-Act) loop pattern integrated with the 4-layer governance model.
...
### WF-RR-04: Service Escrow Milestone Review

```yaml
workflow_id: WF-RR-04
name: Service Escrow Milestone Review
trigger:
  event: MilestoneSubmitted (M009 service escrow)
governance_layer: Layer 1 (advisory only)

ooda_loop:
  observe:
    - fetch_milestone_details(agreement_id, milestone_index)
    - fetch_deliverable_iri(evidence)
    - fetch_prior_milestones(agreement_id)
    - fetch_service_specification(agreement_id)

  orient:
    - evidence_validation (IRI resolves, content matches milestone spec)
    - deliverable_quality (methodology compliance, completeness)
    - consistency_check (cross-reference with prior milestone outputs)
    - scope_assessment (deliverable matches agreed scope)

  decide:
    - high_quality (score > 0.8): recommend_approve
    - needs_work (score 0.4-0.8): recommend_revision(specific_gaps)
    - low_quality (score < 0.4): flag_for_client_attention

  act:
    - submit_review_to_koi (score, recommendation, rationale)
    - notify_client (summary, recommendation)
    - notify_provider (if revision suggested, with specific feedback)

sla:
  response_time: 4h from milestone submission
...
### WF-MM-04: Curation Quality Monitoring & Scoring

```yaml
workflow_id: WF-MM-04
name: Curation Quality Monitoring & Scoring
trigger:
  event: SellOrderCreated OR periodic(daily) OR CollectionBatchAdded (M011)
governance_layer: Layer 1 (scoring) / Layer 2 (challenges)

ooda_loop:
  observe:
    - fetch_batch_metadata (credit class, project, vintage)
    - fetch_reputation_scores (M010: project, class, seller)
    - fetch_price_context (class median, recent trades)
    - fetch_verification_history (last attestation timestamp)

  orient:
    - compute_quality_factors:
        project_reputation: query_m010(subject=project)
        class_reputation: query_m010(subject=class)
        vintage_freshness: linear_decay(batch.start_date, 10y)
        verification_recency: linear_decay(last_verification, 3y)
        seller_reputation: query_m010(subject=seller)
        price_fairness: deviation_from_median(listing_price, class_median)
        additionality_confidence: koi_methodology_analysis(class.metadata_iri)
    - compute_weighted_score (0-1000)
    - compare_to_collection_criteria (if batch in collection)

  decide:
    - quality_score >= collection.quality_floor: status_ok
    - quality_score < collection.quality_floor AND confidence > 0.9:
        flag_quality_degradation
    - quality_score < collection.quality_floor * 0.5 AND confidence > 0.9:
        recommend_auto_challenge (Layer 2: human confirmation)
    - score_delta > 200 from prior score: alert_curator

  act:
    - submit_quality_score_to_contract(batch_denom, score, confidence, factors)
    - update_koi_quality_index
    - notify_curator (if quality degradation)
    - create_challenge_proposal (if auto-challenge threshold met, human confirms)
    - generate_weekly_collection_reports
    - compute_class_price_medians (hourly aggregation)

sla:
  scoring_latency: 30m for new listings, 24h for daily refresh
  authority: scoring=autonomous, challenge=human_confirmed
```
...
|-------|-----------|------------------|-------------|
| Registry Reviewer | 4 (incl. WF-RR-04 M009 milestone review) | Layer 1-2 | KOI + Ledger |
| Governance Analyst | 3 | Layer 1 | Ledger + KOI |
| Market Monitor | 4 (incl. WF-MM-04 M011 curation scoring) | Layer 1-2 | Ledger + M010 |
| Validator Monitor | 3 | Layer 1-3 | Ledger |

**Total Workflows**: 14
**Average Automation**: ~75% (Layer 1-2)
**Human Escalation Points**: 10 defined triggers
