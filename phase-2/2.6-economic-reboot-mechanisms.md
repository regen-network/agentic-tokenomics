...
> **OQ-M013-3**: Should fees be collected in REGEN only, or also in the transaction's settlement currency (USDC, EEUR) with conversion? REGEN-only is simpler but may create friction for non-REGEN transactions.
...
ACTIVE → PROBATION
  ...
  action: issue warning, set probation_period(30 days), reduce block rewards
...
| Proposal Submission | 0.10 | Rewards initiative and leadership |
...
5. **No Double-Counting**: Each transaction counted once for reward scoring; marketplace trades count for buyer OR seller, not both
...
| M013 (Fee Routing) | New | Medium | Critical | x/ecocredit | High — concept agreed; distribution split needs resolution |
